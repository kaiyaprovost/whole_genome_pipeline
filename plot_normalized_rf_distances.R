...
files = list.files(path="/Users/kprovost/Dropbox (AMNH)/Dissertation/CHAPTER2_GENOMES/phydist_rfdist/",
                   pattern="phydist_rfdist.*NORMALIZED.txt$",full.names = T)
...
## combine the RF distances together to generate an average? 

aggM = aggregate(allspp$value~allspp$row+allspp$col,FUN=function(x){mean(x,na.rm=T)})
aggS = aggregate(allspp$value~allspp$row+allspp$col,FUN=function(x){sd(x,na.rm=T)})

aggM_d = as.matrix(spaa::list2dist(aggM))
aggS_d = as.matrix(spaa::list2dist(aggS))

order=c(1,"1A","1B",2:4,"4A",5:28,"LG2","LG5","LGE22","mtDNA","Z")

aggM_d=aggM_d[order,order]
aggS_d=aggS_d[order,order]

pdf("RFdist_averaged_across_species_mean_sd.pdf")
customcorrplot(as.matrix(aggM_d[order,order]),is.corr=F,diag=T,
               main="",method="color",na.label=" ",col=c(rep(rgb(0,0,0,0),44),spectralpal(45)),
               order="original",tl.offset=0.1,cl.lim=c(0,1),
               cl.length=5,
               mar=c(0,0,0,0))
customcorrplot(as.matrix(aggS_d[order,order]),is.corr=F,diag=T,
               main="",method="color",na.label=" ",col=c(rep(rgb(0,0,0,0),44),spectralpal(45)),
               order="original",tl.offset=0.1,cl.lim=c(0,0.2),
               cl.length=5,
               mar=c(0,0,0,0))
dev.off()

png("rfdist_stdev.png")
customcorrplot(as.matrix(aggS_d[order,order]),is.corr=F,diag=T,
               main="",method="color",na.label=" ",col=c(rep(rgb(0,0,0,0),44),spectralpal(45)),
               order="original",tl.offset=0.1,cl.lim=c(0,0.2),
               tl.col="black",cl.col="black",
               cl.length=5,
               mar=c(0,0,0,0))
dev.off()
