...
pdf("hzar_cline_vs_fst_pval.pdf")
par(mfrow=c(2,2),mar=c(4,4,1.5,1.5))
plot(FST,mw,ylab="Mean Cline Width",ylim=c(5,17),xlim=c(0.01,0.13),
     type="n",col="red")
points(newdf$MEAN_FST,newdf$MW,col="black",pch=0)
#mw_mod=lm(mw~FST)
#abline(mw_mod)
mw_mod2=lm(newdf$MW~newdf$MEAN_FST)
...
plot(FST,sw,ylab="S.D. Cline Width",col="red",xlim=c(0.01,0.13), type="n")
points(newdf$MEAN_FST,newdf$SW,ylab="S.D. Cline Width",col="black",pch=0)
sw_mod=lm(sw~FST)
#abline(sw_mod)
...
plot(FST,mc,ylab="Mean Cline Center",col="red",xlim=c(0.01,0.13), type="n")
points(newdf$MEAN_FST,newdf$MC,col="black",pch=0)
#mc_mod=lm(mc~FST)
#abline(mc_mod)
mc_mod2=lm(newdf$MC~newdf$MEAN_FST)
...
plot(FST,sc,ylab="S.D. Cline Center",col="red",xlim=c(0.01,0.13),type="n")
points(newdf$MEAN_FST,newdf$SC,col="black",pch=0)
#sc_mod=lm(sc~FST)
#abline(sc_mod)
## GOES FROM SIG TO NEARLY SIG
sc_mod2=lm(newdf$SC~newdf$MEAN_FST)
abline(sc_mod2,col="red")
mtext(paste("adjR2 =",round(summary(sc_mod)$adj.r.squared,2)))
