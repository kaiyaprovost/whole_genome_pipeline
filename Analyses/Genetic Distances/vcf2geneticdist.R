...
specieslist =c("cur","cri","bru","bil","fla",
               "fus","mel","nit","sin","bel")
...
if (calcDist == T) {
  files=list.files(pattern="vcf.+converted$",recursive = T) 
  ...
  head(files)
  for (vcffile in files[1:length(files)]) {
    print(vcffile)
    ...
    if(file.exists(outfile) && overwrite==F){
      ...
      png(paste(vcffile,"_distances.png",sep=""),
