...
for(sonfile in listsonfiles) {
  ...
  specificfile = tempchrom[length(tempchrom)]
  
  chrom=strsplit(specificfile,"_")[[1]][2]
  ...
  #tempspp = strsplit(sonfile,"/")[[1]]
  spp = strsplit(specificfile,"_")[[1]][1]
  ...
  if(nrow(sonb) <=0){
    ...
    if(file.exists(pngfile) && overwrite==F) {
      ...
      closest_snp = min(as.numeric(as.character(son$position)),na.rm=T)
      furthest_snp = max(as.numeric(as.character(son$position)),na.rm=T)
